...
* Official download: [www.highcharts.com/download](https://www.highcharts.com/download)
* Licensing: [www.highcharts.com/license](https://www.highcharts.com/license)
...
1. Make sure the report is accompanied by a reproducible demo. The ideal demo is created by forking [our standard jsFiddle](https://jsfiddle.net/highcharts/llexl/), adding your own code and stripping it down to an absolute minimum needed to demonstrate the bug.
* Always add information on what browser it applies to, and other information needed for us to debug.
* It may be that the bug is already fixed. Try your chart with our latest work from https://github.highcharts.com/master/highcharts.js before reporting.
...
## Apply a fix
When an issue is resolved, we commit a fix and mark the issue closed. This doesn't mean that a new release is available with the fix applied, but that it is fixed in the development code and will be added to the next stable release. Stable versions are typically released every 1-3 months. To try out the fix immediately, you can run https://github.highcharts.com/highcharts.js or https://github.highcharts.com/highstock.js from any website, but do not use these URLs in production.
...
* Now the patch is on your clipboard, open another tab at [i-tools.org/diff](https://i-tools.org/diff).
* Under "Original file", click "By URL" and enter `https://code.highcharts.com/highcharts.src.js` or another source file from the latest stable release, see [code.highcharts.com](https://code.highcharts.com).
...
* Go to the [Closure Compiler web app](https://closure-compiler.appspot.com/home).
